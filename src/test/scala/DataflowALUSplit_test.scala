...
class DataflowALUSplit_tester extends BasicTester{
    val dut = Module(new DataflowALUSplit(true))
    ...
    printf("cntr = %d, pc_id = %d, taken = %d, correct_wb = %d, wb = %d, pc_wb = %d \n", 
            cntr, dut.io.fpgatest.pc_ex>>2.U, dut.io.fpgatest.halt, wb_check, wb, dut.io.fpgatest.pc >> 2.U)
    //assert(wb === wb_check)
    when(done) { stop(); stop() } 
}

class DataflowSplitALUTests extends FlatSpec with Matchers {
  "Dataflow" should "pass" in {
    assert(TesterDriver execute (() => new DataflowALUSplit_tester))
  }
} 